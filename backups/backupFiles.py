...
def make_archive(destPath:str, src:str):
    ...
    if Path(src).is_dir():
        ...
        shutil.make_archive(destPath+"/"+dt_string, 'zip', Path(src).parent, Path(src).name) 
...
def checkVersionLimit(backupPath:str, versionsToKeep:int, daysToKeep:int):
    ...
    if not IsNullOrDefault(daysToKeep) and not IsNullOrDefault(versionsToKeep):
        print(f"Both variants of versioning controll was used. appling both,daysToKeep:{daysToKeep}, versionsToKeep:{versionsToKeep}" )
...
def main() -> int:
    ...
    parser.add_argument("--versionCount",'-c', help="number of versions to keep", required=False, type=int)
    parser.add_argument("--deleteOlderThan",'-o', help="keep everything newer than o days", required=False, type=int)
    parser.add_argument("--shallowCompareType",'-s', help="How should files be checked. If set to False, it should do byte-wise compares", required=False, default=True)
    args=parser.parse_args()

    backup(filePath=args.filePath, backupName=args.name, shallowCheck=args.shallowCompareType, versionsToKeep=args.versionCount, daysToKeep=args.deleteOlderThan)
